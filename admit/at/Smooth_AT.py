...
class Smooth_AT(AT):
    ...
    **Output BDPs**
    ...
    def run(self):
        ...
        if self._bdp_in[0] == None:
          raise Exception,"Input SpwCube required!"
        bdpnames=[]
        ...
        for ibdp in self._bdp_in:
          ...
          CC = 299792458.0 # speed of light  @todo somewhere else
          ...
          if(velres['unit'] == 'pixel'):
            ...
            velres['unit'] = 'km/s'
          elif( (velres['unit']=='hz') | (velres['unit'] == 'mhz') \
                  |(velres['unit']=='khz') | (velres['unit']=='ghz')):
            hertz_input = True
          ...
          if velres['value'] > 0:
            ...
            # NB: there is apparently a bug in CASA. only smoothing along the frqeuency
          ...
          if convolve_to_min_beam:
            ...
            # for whatever reason, setrestoringbeam does not use the same 
            # set of hashes...
...
#       now let's do the velocity axis
